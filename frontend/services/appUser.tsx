...
export const LoginUser = async (loginUser: LoginDto) => {
    try{
        ...
        console.log("LoginDriver:", error.response.data);
        Alert.alert("Something went wrong");
    }
}

export const LoginDriver = async(loginUser:LoginDto) => {
    try{
        const data = await axios.post<newUserGet>(api + "LoginDriver", loginUser);
        return data.data;
    }catch(error:any){
        console.log("LoginDriver:", error.response.data);
        Alert.alert("Something went wrong");
